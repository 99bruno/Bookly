...
from app.keyboards.book_a_lesson.book_a_lesson import confirm_book_lessons_keyboard, share_contact_keyboard
from app.keyboards.check_schedules.check_schedules import *

from app.database.requests.book_a_lesson.check_dancer import (check_user_registered,
                                                              add_user,
...
from app.states.check_schedules.register_dancer import UserRegistration, Couple
...
async def handle_couple_selection(callback_query: types.CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        lessons = await get_booked_lessons_by_couple(data["couples"][int(couple_id)]["couple_id"])

        await state.update_data(lessons=lessons)

        await callback_query.message.answer(format_string(check_schedules_message,
                                                          [
                                                              data["couples"][int(couple_id)]["dancer1_full_name"] + " та " +
...
async def cancel_booking_handler(query: types.CallbackQuery, state: FSMContext) -> None:
    try:
        couple_info = await check_couple_registered(await check_user_registered(query.from_user.id))
        ...
        await query.message.edit_text(format_string(choose_couple_message, ["\n".join(format_couple(couples))]))
        await query.message.edit_reply_markup(reply_markup=create_keyboard_for_choose_couple_for_schedule(couples))
    except Exception as e:
...
async def cancel_booking_handler(query: types.CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        await query.message.edit_reply_markup(reply_markup=create_keyboard_for_cancel_lesson(sorted_lessons :=
                                                                                             await sort_lessons(
                                                                                                 data["lessons"])))
...
async def cancel_booking_handler(query: types.CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        await query.message.edit_text(format_string(check_schedules_message,
                                                   [
                                                       data["couples"][int(data["couple_id"])]["dancer1_full_name"] + " та " +
...
async def handle_cancel_reason(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(format_string(cancel_lesson_confirmation_message,
                                           [
                                               data["lesson_date"]
                                           ]),
...
async def handle_reschedule_lesson(callback_query: types.CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        if response:
            await callback_query.message.answer(reschedule_lesson_confirmation_message, reply_markup=start_keyboard)
            await state.clear()
        else:
            await callback_query.answer("Виберіть інший урок, цей нажаль зайнятий(", show_alert=True)
