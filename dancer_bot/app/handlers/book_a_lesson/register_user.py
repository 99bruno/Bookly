...
from app.database.requests.book_a_lesson.check_dancer import check_user_registered, add_user, check_couple_registered, check_user_registered_by_phone, add_couple
...
from app.states.book_a_lesson.register_dancer import UserRegistration, RegisterCouple, Couple
...
async def command_book_a_lesson_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if not re.compile(r"^[a-zA-Z'-]+$").match(coach_name := message.text):
            await message.answer("<b>Упс, здається, щось не так із форматом введення імʼя 😕</b>\n\n"
                                 "Перевір себе 👀:\n"
...
async def command_book_a_lesson_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if not re.compile(r"^[a-zA-Z'-]+$").match(coach_surname := message.text):
            await message.answer("<b>Хмм, здається щось не так із форматом введення прізвища 🤔</b>\n\n"
                                 "Перевір себе 👀:\n"
...
async def command_book_a_lesson_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(change_partner_phone_message, reply_markup=types.ReplyKeyboardRemove())
    except Exception as e:
...
async def handle_change_couple(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if len(phone) != 12 or not phone.isdigit():
            await message.answer("Упс, здається у тебе помилка у форматі введення номеру телефону 🫠\n\n"
        ...
        if dancer2_id:
            ...
            if not await check_couple_exists(dancer1_id := await check_user_registered(message.from_user.id), dancer2_id):
                await add_couple(dancer1_id, dancer2_id)

                await state.update_data(couples=await check_couple_registered(dancer1_id))
...
async def handle_change_couple(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await add_user(None, None, data["phone_number"], data["name"], data["surname"],
...
async def choose_partner(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await state.update_data(couples=await check_couple_registered(await check_user_registered(message.from_user.id)))
