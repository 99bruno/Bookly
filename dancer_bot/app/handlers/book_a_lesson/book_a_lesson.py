...
async def handle_change_couple(callback_query: types.CallbackQuery) -> None:
    try:
        await callback_query.message.answer(
            change_partner_solo_or_couple_message,
            reply_markup=change_partner_solo_or_couple_keyboard,
        )
    except Exception as e:
        with sentry_sdk.configure_scope() as scope:
...
async def handle_coach_selection(
    ...
    try:
        ...
        dates, lesson_restrictions, booked_lessons_count = await get_lessons_by_coach(
            coach_id, data["couples"][int(data["couple_id"])]["couple_id"]
        )
        ...
        if not bool(dates):
            await callback_query.answer(
                "На жаль, наразі в цього тренера нема вільних годин для занять :(",
                show_alert=True,
            )
            return
