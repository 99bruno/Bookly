...
from app.database.requests.get_help_from_manager.get_help_from_manager import (
    get_event_info,
)
...
async def command_check_camp_info_handler(
    ...
    try:
        ...
        await message.answer(
            await check_camp_info_message_unpack(
                [
                    event_info.name,
...
async def command_coaches_list_handler(message: types.Message) -> None:
    try:
        await message.answer(
            coaches_program_choose_message, reply_markup=coaches_program_choose_keyboard
        )
    except Exception as e:
        with sentry_sdk.configure_scope() as scope:
            scope.set_extra("user_id", message.from_user.id)
...
async def command_coaches_list_handler(
    ...
    try:
        ...
        await query.message.edit_text(
            await coaches_list_message_unpack(program, coaches_list_message)
        )
        await query.message.edit_reply_markup(
            reply_markup=create_keyboard_for_coaches_camp_info(program)
        )
...
async def command_coaches_list_handler(
    ...
    try:
        ...
        await query.message.edit_text(
            await coaches_list_message_unpack(coaches, coaches_list_message)
        )
        await query.message.edit_reply_markup(
            reply_markup=create_keyboard_for_coaches_camp_info(coaches)
        )
...
async def coach_from_camp_info_handler(
    query: types.CallbackQuery, state: FSMContext
) -> None:
    try:
        ...
        await query.message.edit_text(
            await coach_info_message_unpack(coach, coach_info_message)
        )
...
async def coach_from_camp_info_handler(
    ...
    try:
        ...
        await query.message.edit_text(
            await coach_info_view_price_message_unpack(
                data.get("coach_info"), coach_info_view_price_message
            )
        )
...
async def coach_from_camp_info_handler(
    ...
    try:
        ...
        await query.message.edit_text(
            await coach_info_compare_price_message_unpack(
                coaches, coach_info_compare_price_message, data.get("program")
            )
        )
