...
share_contact_keyboard = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Поширити контакт", request_contact=True)],
    ],
...
def create_keyboard_for_choose_couple(couples: list):
    ...
    for i in range(0, len(couples), 2):
        ...
        if i + 1 < len(couples):
            row.append(
                InlineKeyboardButton(
                    text=f"Пара {i+2}", callback_data=f"couple_{i + 1}"
                )
            )
        keyboard.append(row)

    keyboard.append(
        [InlineKeyboardButton(text="Змінити партнера", callback_data="change_couple")]
    )
...
def create_keyboard_for_coaches(coaches: list, couple_id: int):
    ...
    keyboard.append(
        [InlineKeyboardButton(text="Назад 🔙", callback_data=f"couple_{couple_id}")]
    )
    ...
    inline_keyboard=[
        [
            InlineKeyboardButton(
                ...
                callback_data="lesson_booking_confirmation",
            )
        ],
        [
            InlineKeyboardButton(
                text="Повернутись в головне меню", callback_data="back_to_main_menu"
            )
        ],
    ]
)
