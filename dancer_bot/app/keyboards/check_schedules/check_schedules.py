...
def create_keyboard_for_choose_couple_for_schedule(couples: list) -> InlineKeyboardMarkup:
    keyboard = []

    for i in range(0, len(couples), 2):
        row = [InlineKeyboardButton(text=f"Пара {i + 1}", callback_data=f'schedules_couple_{i}')]
        if i + 1 < len(couples):
            row.append(InlineKeyboardButton(text=f"Пара {i + 2}", callback_data=f'schedules_couple_{i + 1}'))
        keyboard.append(row)

    keyboard.append([InlineKeyboardButton(text=f"Назад 🔙", callback_data=f'back_to_main_menu')])
...
check_schedules_keyboard = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="Перенести урок", callback_data="reschedule_lesson")
        ],
        [
...
def create_keyboard_for_confirm_reschedule_lesson(lessons: list[dict]) -> InlineKeyboardMarkup:
    ...
    for idx, lesson in enumerate(lessons):
        row = [InlineKeyboardButton(text=f'{lesson["start_time"].strftime("%d.%m  %H:%M")}', callback_data=f'lesson_reschedule{idx}')]
        keyboard.append(row)
...
def create_keyboard_for_choose_schedule(lessons: list):
    ...
    for i in range(0, len(lessons), 2):
        row = [InlineKeyboardButton(text=f"Заняття {i + 1}", callback_data=f'schedules_{i}')]
        if i + 1 < len(lessons):
            row.append(InlineKeyboardButton(text=f"Заняття {i + 2}", callback_data=f'schedules_{i + 1}'))
        keyboard.append(row)
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
