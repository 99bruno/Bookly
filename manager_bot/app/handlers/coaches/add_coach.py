...
async def command_add_new_coach_handler(message: types.Message,
                                        bot: Bot,
...
async def command_add_coach_handler(message: types.Message,
                                    state: FSMContext,
                                    bot: Bot, latest_messages: dict) -> None:
...
async def command_add_coach_name_handler(message: types.Message,
                                         state: FSMContext,
                                         bot: Bot, latest_messages: dict) -> None:
...
async def command_add_coach_surname_handler(message: types.Message,
                                            state: FSMContext,
                                            bot: Bot, latest_messages: dict) -> None:
    ...
    answer = await message.answer(add_program_message, reply_markup=add_coach_program_keyboard)
...
async def command_create_new_event_handler(message: types.Message,
    ...
    await state.update_data(available_dates=dates_of_camp, choose_dates=[], full_schedule=full_schedule)
...
async def process_number_selection(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if callback_query.data.startswith('num_'):
        ...
        if number in available_dates:
            ...
            await state.update_data(avaible_dates=available_dates, choosen_dates=choose_dates)
...
async def command_create_new_event_handler(message: types.Message,
    ...
    answer = await message.answer(add_currency_message, reply_markup=add_currency_keyboard)
...
async def command_create_new_event_handler(message: types.Message,
    ...
    answer = await message.answer(await coach_added_message_unpack(data, coach_added_message),
