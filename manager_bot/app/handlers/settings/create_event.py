...
@restricted
async def command_coaches_handler(message: types.Message, state: FSMContext) -> None:
    try:
        await state.clear()

        await message.answer(
            create_event_message, reply_markup=confirm_create_event_keyboard
        )
...
@restricted
async def decline_handler(message: types.Message, state: FSMContext) -> None:
    try:
...
@restricted
async def add_name_handler(message: types.Message, state: FSMContext) -> None:
    try:
        name = message.text

        if len(name) > 50:
            answer = await message.answer("The name is too long. Please try again.")
            latest_messages[message.from_user.id] = (
                answer.message_id,
                message.message_id,
            )
            return
...
@restricted
async def add_date_handler(message: types.Message, state: FSMContext) -> None:
    try:
        dates = message.text
        ...
        await state.update_data(
            start_time=None, end_time=None, lesson_duration=None, breaks=None
        )

        await state.set_state(NewEvent.type_of_schedule)

        await message.answer(
            add_schedule_message, reply_markup=type_of_schedule_keyboard
        )
        ...
        if callback_query.data == "single_template":
            ...
            await callback_query.message.answer(
                single_template_message_unpack(
                    template=single_template_message, data=dates
                ),
            ...
            await callback_query.answer(
                "Ops.. We are currently working on this function, you’ll be able to use it soon!",
                show_alert=True,
            )
...
@restricted
async def single_template_handler(
    callback_query: types.CallbackQuery, state: FSMContext
) -> None:
    try:
        ...
        if callback_query.data == "single_event_start_time":
            ...
            await callback_query.message.answer(
                single_event_breaks_message_unpack(
                    data["breaks"], single_event_breaks_message
                ),
            ...
            if not validate_all_parameters(data):
                ...
                await callback_query.message.answer(
                    single_template_message_unpack(
                        template=single_template_message, data=data
                    ),
                    reply_markup=single_template_keyboard,
                )
                ...
                data_correctness, schedule = generate_schedule(
                    start_time, end_time, lesson_duration, breaks
                )
                ...
                await callback_query.message.answer(
                    created_schedule_message_unpack(created_schedule_message, schedule)
                )
...
@restricted
async def add_start_time_handler(message: types.Message, state: FSMContext) -> None:
    try:
...
@restricted
async def add_end_time_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if start_time and end_time_answer <= start_time:
            await message.answer(
                "End time must be greater than start time. Please try again."
            )
            return
...
@restricted
async def add_lesson_duration_handler(
...
async def add_breaks_handler(
    ...
    try:
        if callback_query.data == "add_break":
            ...
            data = await state.get_data()
            await callback_query.message.answer(
                single_template_message_unpack(single_template_message, data),
                reply_markup=single_template_keyboard,
            )
...
@restricted
async def add_breaks_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if start_time is None or end_time is None:
            await message.answer(
                "Start time and end time must be set before adding breaks. Please try again."
            )
            await state.set_state(NewEvent.single_template)
            await message.answer(
                single_template_message_unpack(
        ...
        if new_break_start < start_time_dt or new_break_end > end_time_dt:
            await message.answer(
                "Break time must be within the start and end times. Please try again."
            )
            return
        ...
        if data["breaks"] is None:
            ...
            for existing_break in breaks:
                ...
                existing_break_start = datetime.strptime(
                    existing_break_start_str, "%H:%M"
                )
                existing_break_end = datetime.strptime(existing_break_end_str, "%H:%M")

                if (
                    new_break_start < existing_break_end
                    and new_break_end > existing_break_start
                ):
                    await message.answer(
                        "Break times cannot overlap. Please try again."
                    )
                    return
        ...
        await message.answer(
            single_template_message_unpack(template=single_template_message, data=data),
            reply_markup=single_template_keyboard,
        )
...
@restricted
async def add_description_handler(message: types.Message, state: FSMContext) -> None:
    try:
        description = message.text

        if len(description) > 1000:
            answer = await message.answer(
                "The description is too long. Please try again."
            )
            return
        ...
        answer = await message.answer(
            single_event_created_message_unpack(event_created_message, data),
