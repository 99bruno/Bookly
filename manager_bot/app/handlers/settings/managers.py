...
@restricted
async def edit_managers_handler(message: types.Message,
                                state: FSMContext):
    try:
...
@restricted
async def add_manager_handler(message: types.Message,
                              state: FSMContext):
    try:
...
@restricted
async def added_manager_handler(message: types.Message,
                                state: FSMContext):
    try:
...
@restricted
async def remove_manager_handler(message: types.Message,
                         state: FSMContext):
    try:

        await message.answer(await remove_managers_msg_unpack(managers := await get_managers(), remove_managers_msg),
...
@restricted
async def remove_manager_handler(query: types.CallbackQuery,
                                 state: FSMContext):
