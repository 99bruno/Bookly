...
@router.callback_query(
    lambda event: event.data == "latin_book_lesson"
    or event.data == "ballroom_book_lesson"
)
async def handle_program_selection(
    ...
    try:
        ...
        await callback.message.edit_reply_markup(
            reply_markup=create_keyboard_for_coaches(coaches, data["couple_id"])
        )
...
async def handle_coach_selection(
    ...
    try:
        ...
        dates = await get_lessons_by_coach(coach_id, data["couple_id"])
        ...
        if not bool(dates):
            await callback_query.answer(
                "Unfortunately, this trainer currently has no available hours for classes :(",
                show_alert=True,
            )
            return
    ...
    try:
        ...
        if callback_query.data.startswith("date_"):
            ...
            await callback_query.message.edit_text(
                "Now choose a convenient time for you 🤩"
        ...
        if callback_query.data == "book_lesson_to_confirm":
            ...
            await callback_query.message.edit_text(
                format_lesson_info(data_of_lessons, confirm_book_lessons_message)
            )
            await callback_query.message.edit_reply_markup(
...
async def command_book_lessons_handler(
    callback_query: types.CallbackQuery, state: FSMContext
) -> None:
    try:
        data = await state.get_data()

        unavailable_lessons = await book_lessons(
            data["selected_dates"], data["couple_id"], data["coach_id"]
        )
        ...
        if unavailable_lessons:
            await callback_query.answer(
                unavailable_lessons_message.format(", ".join(unavailable_lessons)),
                show_alert=True,
            )
...
async def return_to_schedule(callback_query: types.CallbackQuery, data) -> None:
    try:
        schedule = await get_booked_lessons_for_couple(data["couple_id"])

        await callback_query.message.answer(
            await couple_info_message_unpack(
                {"name": data["couples_info"][data["current_couple_id"]]["name"]},
                couple_info_message,
                schedule,
            ),
