...
async def camp_settings_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(create_couple_phone_message, reply_markup=types.ReplyKeyboardRemove())
...
async def phone_1_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if len(phone) != 12 or not phone.isdigit():
            await message.answer("Oops, you seem to have an error in the phone number input format 🫠\n\n"
    ...
    try:
        ...
        if dancer_id := await check_user_registered_by_phone(phone):
            ...
            if await check_couple_exists(data["dancer_id_1"], dancer_id):
                await message.answer("This pair already exists in the system 🤔\n\n"
                ...
                await message.answer("What would you like to do next?", reply_markup=start_keyboard)
        else:
            await message.answer(create_dancer_name_message)
...
async def name_1_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if not re.compile(r"^[a-zA-Z'-]+$").match(coach_name := message.text):
            await message.answer("<b>Oops, something seems wrong with the name input format 😕</b>\n\n"
                                 "Check yourself 👀:\n"
...
async def surname_1_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        if not re.compile(r"^[a-zA-Z'-]+$").match(message.text):
            await message.answer("<b>Hmm, something seems wrong with the last name input format 🤔</b>\n\n"
                                 "Check yourself 👀:\n"
...
async def surname_1_handler(message: types.Message, state: FSMContext) -> None:
    try:
        ...
        await add_couple(await check_user_registered_by_phone(data["phone_1"]),
