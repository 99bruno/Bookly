...
from app.keyboards.camp_settings.coaches_settings.view_and_edit import *
from app.keyboards.start.start import start_keyboard

from app.scripts.coaches.coaches import *

from app.templates.camp_settings.coaches_settings.view_and_edit import *
from app.templates.start.start import start_message

from app.database.requests.coaches.coaches import *

from app.states.coaches_settings.view_and_edit import ListOfCoaches
...
async def command_coaches_list_handler(message: types.Message,
...
async def callback_return_to_coaches_handler(callback_query: types.CallbackQuery,
                                             state: FSMContext) -> None:
    try:
...
async def callback_return_to_coaches_handler(callback_query: types.CallbackQuery,
    ...
    try:
        ...
        await callback_query.message.answer(start_message, reply_markup=start_keyboard)
...
async def callback_coach_info_handler(callback_query: types.CallbackQuery,
                                      state: FSMContext) -> None:
    try:
        ...
        await callback_query.message.edit_text(coach_unpack_info(coach_info, coach_info_message))
...
async def callback_return_to_coaches_handler(callback_query: types.CallbackQuery,
    ...
    try:
        ...
        await callback_query.message.edit_text(coaches_unpack_info(coaches["coaches_info"], coaches_list_message))
        await callback_query.message.edit_reply_markup( reply_markup=coaches_list_keyboard(coaches["coaches_info"]))
...
async def callback_return_to_coaches_handler(callback_query: types.CallbackQuery,
    ...
    try:
        ...
        await callback_query.message.edit_text(coach_unpack_info_for_edit(coach_info, coach_info_message))
...
async def callback_return_to_coaches_handler(callback_query: types.CallbackQuery,
    ...
    try:
        ...
        if callback_query.data == 'edit_coach_name':
            ...
            await callback_query.message.edit_text("Interesting changes! Choose another program 💃🏼")
...
async def edit_coach_name_handler(message: types.Message,
                                  state: FSMContext) -> None:
    ...
    try:
        ...
        await update_coach_info(coach_info["coaches_info"][coach_info["current_coach"]]["id"], "firstname", coach_name)
        ...
        coach_info["coaches_info"][coach_info["current_coach"]]["coach"] = coach_name + " " + surname

        await state.update_data(coaches_info=coach_info["coaches_info"])

        await message.answer("Coach name has been updated")
        await state.set_state(ListOfCoaches.edit)
        await message.answer(coach_unpack_info(coach_info["coaches_info"][coach_info["current_coach"]], coach_info_message), reply_markup=coach_info_keyboard)
...
async def edit_coach_surname_handler(message: types.Message,
                                     state: FSMContext) -> None:
    ...
    try:
        ...
        await update_coach_info(coach_info["coaches_info"][coach_info["current_coach"]]["id"], "lastname", coach_surname)
        ...
        coach_info["coaches_info"][coach_info["current_coach"]]["coach"] = name + " " + coach_surname
...
async def edit_coach_program_handler(callback_query: types.CallbackQuery,
                                   state: FSMContext) -> None:
    try:
        ...
        await update_coach_info(coach_info["coaches_info"][coach_info["current_coach"]]["id"], "program", coach_program)
        ...
        await state.set_state(ListOfCoaches.edit)
        await callback_query.message.answer(coach_unpack_info(coach_info["coaches_info"][coach_info["current_coach"]], coach_info_message),
...
async def edit_coach_price_handler(message: types.Message,
    ...
    try:
        ...
        await update_coach_info(coach_info["coaches_info"][coach_info["current_coach"]]["id"], "price", coach_price)
        ...
        await state.set_state(ListOfCoaches.edit)
        await message.answer(coach_unpack_info(coach_info["coaches_info"][coach_info["current_coach"]], coach_info_message),
                             reply_markup=coach_info_keyboard)
...
@router.callback_query(lambda c: c.data == "USD" or c.data == "EUR" or c.data == "UAH" or c.data == "GBP")
async def edit_coach_currency_handler(callback_query: types.CallbackQuery,
                                   state: FSMContext) -> None:
    ...
    try:
        ...
        await update_coach_info(coach_info["coaches_info"][coach_info["current_coach"]]["id"], "currency", id_currency)
        ...
        await state.set_state(ListOfCoaches.edit)
        await callback_query.message.answer(coach_unpack_info(coach_info["coaches_info"][coach_info["current_coach"]],
                                                              coach_info_message),
                                            reply_markup=coach_info_keyboard)
...
async def callback_return_to_coaches_handler(callback_query: types.CallbackQuery,
    ...
    try:
        ...
        await callback_query.message.edit_reply_markup(reply_markup=create_keyboard_for_dates(
...
async def callback_view_schedule_handler(callback_query: types.CallbackQuery,
                                         state: FSMContext) -> None:
    try:
        ...
        if callback_query.data == "return_to_coach_info":
            data = await state.get_data()
            await state.set_state(ListOfCoaches.coach)
            await callback_query.message.edit_text(coach_unpack_info(data["coaches_info"][data["current_coach"]],
                                                                     coach_info_message))
            await callback_query.message.edit_reply_markup(reply_markup=coach_info_keyboard)
            return
            ...
            await callback_query.message.edit_reply_markup(reply_markup=create_keyboard_for_dates(data["coaches_info"][data["current_coach"]]["dates"]))
            return
