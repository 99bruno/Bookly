        ...
        return ("<b>Coaches booking rating:</b>\n\n"
         "<blockquote>") + "\n".join(
            [
                ...
                for idx, coach in enumerate(sorted(coaches, key=lambda x: x[2] / (x[3] - (x[4] - x[2])), reverse=True)) if coach[3] - (coach[4] - coach[2]) > 0]
        ) + "</blockquote>"
...
async def analysis_overall_income_unpack(income: list) -> str:
    return ("<b>Overall income:</b>\n\n"
            ...
            f"\n\n<b>Paid lessons:</b>\n\n"
            f"<blockquote>"
            f"{"\n".join([f"• {currency[3]}/{currency[1]} {currency[0]} ~ {currency[3]/currency[1] * 100:.1f}%" for currency in income])}"
            "</blockquote>"
            )
