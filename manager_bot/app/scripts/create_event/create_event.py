...
def generate_schedule(
    start_time: str, end_time: str, lesson_duration: int, breaks: list
) -> (bool, list):
    ...
    while current_time + lesson_duration_td <= end_time_dt:
        ...
        if not overlap:
            schedule.append(
                f"{lesson_start.strftime('%H:%M')}-{lesson_end.strftime('%H:%M')}"
            )
...
def single_event_created_message_unpack(template: str, data: dict) -> str:
    return template.format(
        ...
        "\n".join(
            [
                f"<b>•Lesson</b> {idx+1}: {day_schedule}"
                for idx, day_schedule in enumerate(data["final_schedule"])
            ]
        ),
    )
...
def single_event_breaks_unpack(breaks: (list, None)) -> str:
    return (
        ...
        + "\t\t\t".join(
            [f"•Break {idx+1}: {break_time}\n" for idx, break_time in enumerate(breaks)]
        )
    )


def single_event_breaks_message_unpack(breaks: (list, None), template: str) -> str:
...
def created_schedule_message_unpack(
    ...
    return template.format(
        "\n".join(
            [
                f"•Lesson {idx+1}: {day_schedule}"
                for idx, day_schedule in enumerate(schedule)
            ]
        )
    )
