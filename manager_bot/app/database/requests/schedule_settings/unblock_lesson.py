...
from app.database.models import Coach, Lesson, ScheduleEvent, BookedLesson, async_session
...
async def get_coach_by_a_date(date_of_camp: str) -> dict[str: int]:
    async with async_session() as session:
        result = await session.execute(
            select(Lesson, Coach.full_name, Coach.id).distinct().join(Coach, Lesson.id_coach == Coach.id).where(
...
async def get_lessons_by_a_coach_and_date_and_not_booked(coach_id: int, date_of_camp: str) -> dict[str, Any]:
    async with async_session() as session:
        result = await session.execute(
            select(Lesson.id, Lesson.start_time).where(
                ...
                ~Lesson.id.in_(select(BookedLesson.id_lesson))
            )
        )
...
async def unblock_lesson_by_id(lesson_id: int) -> None:
    async with async_session() as session:
