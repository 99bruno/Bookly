...
from app.database.models import async_session, Lesson, BookedLesson, Couple, Dancer, Coach
...
async def get_all_couples_with_dancers():
    async with async_session() as session:
        result = await session.execute(
            select(Couple.id, Dancer.id, Dancer.full_name, Dancer.phone, Couple.id_dancer1, Couple.id_dancer2)
            .join(Dancer, (Dancer.id == Couple.id_dancer1) | (Dancer.id == Couple.id_dancer2))
        )
        couples_with_dancers = result.fetchall()

        couples_info = {}
        for couple_id, dancer_id, dancer_name, dancer_phone, id_dancer1, id_dancer2 in couples_with_dancers:
            ...
            if dancer_id == id_dancer1:
                couples_info[couple_id]["dancer1"] = {"name": dancer_name, "phone": dancer_phone}
            elif dancer_id == id_dancer2:
                couples_info[couple_id]["dancer2"] = {"name": dancer_name, "phone": dancer_phone}
