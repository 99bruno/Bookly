...
from app.database.models import async_session, BookedLesson, Lesson, Coach, Couple, Dancer, Currency, Payment, Change
    ...
    async with async_session() as session:
        async with session.begin():
            ...
            await session.execute(insert(Change).values(
                ...
                coach_name=data["lessons"][data["current_couple_id"]]["coach"]["full_name"],
